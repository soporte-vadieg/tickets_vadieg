...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        // Crear el token JWT
        const token = jwt.sign({ id: user.id, username: user.username,role:user.role, email: user.email,full_name:user.full_name ,id_area: user.area_name}, process.env.JWT_SECRET, {
            expiresIn: '1h',
        });
